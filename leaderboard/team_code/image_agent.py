...
class ImageAgent(BaseAgent):
    def setup(self, path_to_conf_file):
        ...
        self.manual_target = np.array([0,0])
    ...
    def tick(self, input_data):
        ...
        print('[INFO] Far Node: {}, GPS: {}'.format(far_node, gps))
        target = R.T.dot(far_node - gps)
        print('[INFO] Target in Car Frame Before Multiply/Add: ', target)
        # Is this some sort of normalization?
        target *= 5.5
        target += [128, 256]
        target = np.clip(target, 0, 256)
        # print('[INFO] Target After: ', target )

        result['target'] = target

        return result

    def get_manual_target(self):
        if self.current_count >= self.target_count:
            self.current_count = 0
            print('Enter Target Count:')
            try:
                self.target_count = int(input())
            except Exception as e:
                self.target_count = 0
            print('Enter Target (in car frame without norm):')
            try: 
                target = np.array([float(x) for x in input().strip().split()])
            except Exception as e:
    ...
    def run_step(self, input_data, timestamp):
        print('[INFO] in run_step')
        if not self.initialized:
            self._init()

        tick_data = self.tick(input_data)

        # Manually Changing Target
        try: 
            tick_data['target'] = self.get_manual_target()
        except Exception as e:
            print(e)
            tick_data['target'] = np.array([0.0,0.0])
